...
class angketEvaluasiController extends Controller
{
    ...
    {
        $this->middleware('auth')->except('proses_penilaian');
...
//            return view('pages.angket.angketEvaluasiTutor', compact('masa'))->with(['error' => 'Data tidak ditemukan']);
    ...
    public function proses_penilaian($masa, $jenis)
    {
        ...
        if ($jenis == 'penilaian') {
            ...
            return redirect()->back()->with(['success' => 'Data ' . $masa . ' Penilaian berhasil dilakukan']);
        }
        if ($jenis == 'jadwal') {
            $id = $masa;
//            $masa = angketTutorModel::getMasaIjinKelas();
            $data_array = QuerySRS5G::getttmbymasa($id);
            ...
            if (!empty($data_array)) {
                foreach ($data_array as $items) {
                    ...
                    if (empty($cek_data)) {
                        ...
                        if ($masa == $cek_data[0]->masa && $kelas == $cek_data[0]->kelas) {
                            $id = $cek_data[0]->id;
                            $query = jadwal_tutorialModel::update_jadwal_ttm($id, $array);
                        }
                    }
                }
            }
